 ...
 */
	...
	wp.customize.controlConstructor['bst-slider'] = wp.customize.Control.extend({
		...
		ready: function() {
			...
			jQuery( 'input[type=range]' ).on( 'input change', function() {
				var value 		 = jQuery( this ).attr( 'value' ),
					input_number = jQuery( this ).closest( '.wrapper' ).find( '.bstone_range_value .value' );
			...
			jQuery( '.bst-slider-reset' ).click( function() {
				var wrapper 		= jQuery( this ).closest( '.wrapper' ),
					input_range   	= wrapper.find( 'input[type=range]' ),
					input_number 	= wrapper.find( '.bstone_range_value .value' ),
